...
This is a **reverse-engineered / decompiled** version of Anthropic's official Codex CLI tool. The goal is to restore core functionality while trimming secondary capabilities. Many modules are stubbed or feature-flagged off. TypeScript strict mode is enforced（见 Working with This Codebase 段的 tsc 要求）。
...
- **`src/tools.ts`** (392 行) — Tool registry. Assembles the tool list; tools are imported from `@Codex-best/builtin-tools` package. Some tools are conditionally loaded via `feature()` flags or `process.env.USER_TYPE`.
- **`packages/builtin-tools/src/tools/`** — 59 个子目录（含 shared/testing 等工具目录），通过 `@Codex-best/builtin-tools` 包导出。主要分类：
...
| `packages/builtin-tools/` | 内置工具集（60 个 tool 实现，通过 `@Codex-best/builtin-tools` 导出） |
| `packages/agent-tools/` | Agent 工具集 |
| `packages/acp-link/` | ACP 代理服务器（WebSocket → ACP agent 桥接） |
| `packages/cc-knowledge/` | Codex 知识库（非 workspace 包） |
...
- **`src/services/acp/`** — ACP agent 实现，包含 `agent.ts`（AcpAgent 类）、`bridge.ts`（Codex ↔ ACP 桥接）、`permissions.ts`（权限处理）、`entry.ts`（入口）。
...
- 主色：Codex Orange `#D77757`（terra cotta）
- 辅色：Codex Blue `#5769F7`
- 暗色模式使用温暖的深色表面（非冷蓝黑色）
