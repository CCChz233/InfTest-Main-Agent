...
> 2026-05-29 当前联调唯一口径：本文件中“建议路径 / 请求体与响应体定义 / 主 Agent 应做”为准；所有“当前代码状态”“尚未实现”“建议下一步补齐”均视为历史归档说明，不作为现网协议依据。  
> 当前 `/api/*` 统一 real + 异步受理：成功返回表示请求已受理，最终执行状态统一通过 `GET /tasks/{exec_id}` 查询。
...
```text
POST /api/proxy-plan-task-submit
```
...
```text
POST /api/proxy-update-task-status
```
...
当前 mock 后端已支持 POST /api/proxy-update-task-status。
